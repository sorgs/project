...
public final class UmengClient {
    ...
    public static void init(Application application) {
        ...
        try {
            Bundle metaData = application.getPackageManager()
                                         .getApplicationInfo(application.getPackageName(),
                                                 PackageManager.GET_META_DATA).metaData;
            // 友盟统计，API 说明：https://developer.umeng
            // .com/docs/66632/detail/101814#h1-u521Du59CBu5316u53CAu901Au7528u63A5u53E32
            UMConfigure.init(application, String.valueOf(metaData.get("UMENG_APPKEY")), "umeng",
                    UMConfigure.DEVICE_TYPE_PHONE, "");
            // 初始化各个平台的 Key
            PlatformConfig.setWeixin(String.valueOf(metaData.get("WX_APPID")),
                    String.valueOf(metaData.get("WX_APPKEY")));
            PlatformConfig.setQQZone(String.valueOf(metaData.get("QQ_APPID")),
                    String.valueOf(metaData.get("QQ_APPKEY")));

            //PlatformConfig.setSinaWeibo(String.valueOf(metaData.get("SN_APPID")), String.valueOf(metaData.get
            // ("SN_APPKEY")),"http://sns
            // .whalecloud.com");
     ...
     * @param listener 分享监听
     */
    public static void share(Activity activity, Platform platform, UmengShare.ShareData data,
                             UmengShare.OnShareListener listener) {
        if (isAppInstalled(activity, platform.getPackageName())) {
            new ShareAction(activity)
                    .setPlatform(platform.getThirdParty())
                    .withMedia(data.create())
                    .setCallback(listener != null
                            ? new UmengShare.ShareListenerWrapper(platform.getThirdParty(), listener)
                            : null)
                    .share();
        } else {
            // 当分享的平台软件可能没有被安装的时候
        ...
        if (isAppInstalled(activity, platform)) {
            ...
            try {
                ...
                UMShareAPI.get(activity)
                          .getPlatformInfo(activity, platform.getThirdParty(),
                                  listener != null ? new UmengLogin.LoginListenerWrapper(platform.getThirdParty(),
                                          listener) : null);
